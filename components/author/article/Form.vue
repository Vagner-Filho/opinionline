<template>
  <form class="max-w-4xl flex flex-col justify-center m-auto px-2 h-full max-h-[800px]" @submit.prevent="$emit('article', articleData)">
    ...
    <label for="cover-input" :class="{ 'hover:cursor-pointer' : !isViewOnly }" class="std-author-input dark:bg-[#3b3b3b] w-full h-14 leading-[3.5rem] text-[#9fa6b2] text-center mt-[90px] mb-1 px-0">
      <img v-if="coverPreview" :src="coverPreview" alt="cover-preview" class="object-cover w-full h-full">
      <div v-else>
        {{ coverPlaceholder }}
      </div>
    </label>
    <input ref="coverInput" type="file" id="cover-input" class="hidden" @change="handleFileChange" :disabled="isViewOnly" />
